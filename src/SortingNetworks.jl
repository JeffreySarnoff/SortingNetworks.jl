"""
Sorts 1,2,..,16 values using the best known sorting network for each.

Accepts values of any one type T that supports `(<)(a::T, b::T)`

Returns a tuple containing the sorted values

>>    sort(a::T), sort(a::T, b::T), ..., sort(a::T, b::T,.., p::T)

    sort( 8, 5, 9, 11, 2 ) == ( 2, 5, 8, 9, 11 )
...
#=
    The sixteen sort methods use sorting networks of least work.
    Sorting networks that sort 1,2,.. 16 values are provided.
    ...
    sort(x1::T), sort(x1::T, x2::T), .., sort(x1::T, .., x16::T)

    sort(tup::NTuple{1,T}), .., sort(tup::NTuple{16,T})
=#
...
#=
     sort given a tuple of the values
=#
...
#=
     sort given a vector of the values
=#

function sort(v::Vector{T}, xs...) where T
    n = length(v)
